...
export default function Profiles() {
  ...
  return (
    <section className="mt-12 w-full max-w-3xl">
      ...
      {items.length === 0 ? (
        <div className="mt-0 stext-sm text-gray-600">No profiles yet. Create your first one above.</div>
      ) : (
        <ul className="mt-0 divide-y rounded-xl border bg-white shadow-sm">
          {items.map((p) => (
            <li key={p.id}>
              <Link href="/profiles" className="flex items-start justify-between gap-0 p-2 hover:bg-gray-50 transition">
                <div className="min-w-0">
                  ...
                  <div className="mt-0 text-[11px] text-gray-500">{new Date(p.createdAt).toLocaleString()}</div>
                </div>
              </Link>
            </li>
          ))}
